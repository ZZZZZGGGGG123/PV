...
class Unet(object):
    _defaults = {
        ...
        #-------------------------------------------------------------------#
        "model_path"    : r"E:\code\unet-pytorch-main\best_epoch_weights_1102.pth",
        #--------------------------------#
        #   所需要区分的类的个数+1
        #--------------------------------#
        "num_classes"   : 2,
    ...
    #   检测图片
    #---------------------------------------------------#
    def detect_image(self, image,image_name , count=False, name_classes=None):
        #---------------------------------------------------------#
        ...
        if count:
            ...
            print("\n")
            print("|%25s | %6s | %6s | %6s |" % ("Key", "Value", "Ratio", "Area"))
            print('-' * 79)

            #记录文件文件名
            pv_index = name_classes.index("PV")
            pv_pixels = np.sum(pr == pv_index)
            ...
            for i in range(self.num_classes):
                ...
                area = 262144 * (ratio / 100)  # 修改这里，将 ratio 除以100
                if num > 0:
                    print("|%25s | %6s | %6.2f%% | %6.2f |" % (str(name_classes[i]), str(num), ratio, area))  # 在这里加入 area
                    print('-' * 79)
                classes_nums[i] = num
            pv_index = name_classes.index("PV")
            area_pv =16384 * (classes_nums[pv_index] / total_points_num)
            print("classes_nums:", classes_nums)
            #return area_pv
            #return image, area_pv
        ...
        if self.mix_type == 0:
            ...
            # 创建一个二值化图像，仅包含预测结果
            binary_img = np.zeros((np.shape(pr)[0], np.shape(pr)[1]), dtype='uint8')
            # 假设您的目标类别是1（您可以根据需要更改这个值）
            binary_img[pr == 1] = 255
            # 将NumPy数组转换为PIL图像
            image = Image.fromarray(binary_img, 'L')
