...
def stitch_images(predicted_folder, output_path, original_folder):
    ...
    for base_name, slices in images_dict.items():
        ...
        #构建第一张图像的路径
        first_image_path = os.path.join(predicted_folder, f"{base_name}_0_0.tif")
        ...
        for (i, j), arrs in slices.items():
            for b in range(bands):
                ...
                source_data = arrs[b]  # arrs[b] 包含当前图像块的数据
                source_rows, source_cols = source_data.shape  # 获取源数据的实际尺寸
            
                target_start_row = i * tile_height
                target_start_col = j * tile_width
            
                # 确定实际的目标区域尺寸，防止超出边界
                target_end_row = target_start_row + source_rows
                target_end_col = target_start_col + source_cols
            
                # 动态调整目标区域尺寸以匹配源数组的尺寸
                stitched_images[b][target_start_row:target_end_row, target_start_col:target_end_col] = source_data
        ...
        print(f"{base_name}_stitched.tif已完成")
        # 写入拼接图像数据
        for b in range(bands):
...
if __name__ == "__main__":
    original_folder = r"D:\ceshixiazai3"
    dir_save_path = r"D:\TEST\img_predict_out2"
    stitched_output_folder = r"D:\TEST\img_predict_stitch2"
